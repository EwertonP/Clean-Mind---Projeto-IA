...
export function AgencyFinanceDashboard() {
  ...
  const planDistData = [
    ...
    { name: 'Pro', value: planStats.pro, color: '#818cf8' },
    { name: 'Premium', value: planStats.premium, color: '#c084fc' },
  ];
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
           <div className="flex items-center space-x-2 text-[#C1E2A4] mb-2">
             ...
             <span className="text-[13px] font-bold tracking-wider text-[#192F28] uppercase">Financeiro</span>
           </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
          ...
          <div className="mt-1 flex items-baseline">
            ...
            <span className="ml-2 text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-md">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center space-x-2 text-sm font-medium text-emerald-600 bg-emerald-50 px-3 py-1 rounded-lg">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
            <h3 className="text-lg font-bold text-slate-900 mb-4">Últimos Pagamentos (Assinaturas)</h3>
            <div className="overflow-x-auto custom-scroll">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-100 text-sm">
                    <th className="pb-3 font-semibold text-slate-500">Cliente</th>
                    <th className="pb-3 font-semibold text-slate-500">Plano</th>
                    <th className="pb-3 font-semibold text-slate-500">Data</th>
                    <th className="pb-3 font-semibold text-slate-500">Valor</th>
                ...
                <tbody className="text-sm">
                  {recentPayments.length > 0 ? recentPayments.map((payment, idx) => (
                    <tr key={idx} className="border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-colors">
                      <td className="py-4 font-medium text-slate-900">{payment.name}</td>
                      <td className="py-4">
                        <span className={`px-2 py-1 rounded-md text-xs font-bold uppercase ${
                          payment.plan === 'premium' ? 'bg-purple-100 text-purple-700' : 'bg-indigo-100 text-indigo-700'
                        }`}>
                          {payment.plan}
                        </span>
                      </td>
                      <td className="py-4 text-slate-600">{payment.date}</td>
                      <td className="py-4 font-medium">R$ {payment.amount},00</td>
        ...
        <div className="space-y-6">
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-3 border border-indigo-50 bg-indigo-50/50 rounded-xl">
                <div className="flex items-center space-x-3">
                  <div className="w-3 h-3 rounded-full bg-indigo-400"></div>
                  <div>
                    <p className="text-sm font-bold text-indigo-900">Pro</p>
                    <p className="text-xs text-indigo-700/70">Intermediário</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-indigo-900">{planStats.pro}</p>
              ...
              <div className="flex items-center justify-between p-3 border border-purple-50 bg-purple-50/50 rounded-xl">
                <div className="flex items-center space-x-3">
                  <div className="w-3 h-3 rounded-full bg-purple-400"></div>
                  <div>
                    <p className="text-sm font-bold text-purple-900">Premium</p>
                    <p className="text-xs text-purple-700/70">Pacote Completo</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-purple-900">{planStats.premium}</p>
